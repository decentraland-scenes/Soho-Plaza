  ...
  executeTask(async () => {
    try {
      ...
      const contractAddress =
        version == '1' ? CONTRACTS.mainnet : CONTRACTS.ropsten
      ...
      const txId = await contract.create(name, _data, _palette, {
        from: address,
        value: EthConnect.toWei(
          `${VALUE.amount}`,
          VALUE.unit as
            | 'ether'
            | 'noether'
      ...
      showMessage(
        ...
        'PLEASE CHECK YOUR WALLET FOR DETAILS',
        5000
      )
    } catch (error) {
