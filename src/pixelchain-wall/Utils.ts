...
export function setTimeout(ms: number, callback: () => void) {
    ...
    entity.addComponent(new utils.Delay(ms, () => {
        callback()
 ...
 */
export function addOneTimeTrigger(position: Vector3, size: Vector3, onFirstPlayerEnter: () => void, parent?: Entity, show: boolean = false) {
    let entered = false
