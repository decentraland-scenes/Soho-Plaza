export const data = [
  /// --- Museum A ---
  { // BITCOIN BITCH - TOKEN 5
    code: 1,
    contract_address: "0xb932a70a57673d89f4acffbe830e8ed7f75fb9e0",
    token_id: "4831",
    transform: {
      position: { x: 160 - 85.381, y: 3.169, z: 160 + 126.427 },
      rotation: { x: 0, y: 90 + 197.746, z: 0 },
      scale: { x: 4, y: 4, z: 2 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0xb932a70a57673d89f4acffbe830e8ed7f75fb9e0/4831"
  },
  { // Still Life #4
    code: 2,
    contract_address: "0xb932a70a57673d89f4acffbe830e8ed7f75fb9e0",
    token_id: "6919",
    transform: {
      position: { x: 160 - 85.525, y: 3.169, z: 160 + 119.428 },
      rotation: { x: 0, y: 90 + 174.574, z: 0 },
      scale: { x: 4, y: 4, z: 2 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0xb932a70a57673d89f4acffbe830e8ed7f75fb9e0/8305"
  },
  { // CRYPTO ARTIST
    code: 3,
    contract_address: "0xb932a70a57673d89f4acffbe830e8ed7f75fb9e0",
    token_id: "7754",
    transform: {
      position: { x: 160 - 84.682, y: 3.169, z: 160 + 112.096 },
      rotation: { x: 0, y: 90 + 182.534, z: 0 },
      scale: { x: 4, y: 4, z: 2 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0xb932a70a57673d89f4acffbe830e8ed7f75fb9e0/7754"
  },
  { // FUTURE
    code: 4,
    contract_address: "0x4679b43e6ebcb2e7fca0f4e943e6231b87cf29f7",
    token_id: "21",
    transform: {
      position: { x: 160 - 87.012, y: 3.566, z: 160 + 104.805 },
      rotation: { x: 0, y: 90 + 204.917, z: 0 },
      scale: { x: 8, y: 8, z: 4 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0x4679b43e6ebcb2e7fca0f4e943e6231b87cf29f7/21"
  },
  { // Coffee love 1/1
    code: 5,
    contract_address: "0x22adcaf1d6b5213a76bfddab6d4d38c25ce7c029",
    token_id: "48",
    transform: {
      position: { x: 160 - 91.037, y: 3.169, z: 160 + 97.799 },
      rotation: { x: 0, y: 90 + 214.139, z: 0 },
      scale: { x: 4, y: 4, z: 2 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0x22adcaf1d6b5213a76bfddab6d4d38c25ce7c029/48"
  },
  { // I can't wait #6/23
    code: 6,
    contract_address: "0xfd385556ebf6b183380afcd61efd8194d0d46520",
    token_id: "7",
    transform: {
      position: { x: 160 - 99.237, y: 3.566, z: 160 + 91.226 },
      rotation: { x: 0, y: 90 + 253.762, z: 0 },
      scale: { x: 6, y: 6, z: 3 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0xfd385556ebf6b183380afcd61efd8194d0d46520/7"
  },
  { // Out of Many, One - Original #1
    code: 7,
    contract_address: "0xecf7ef42b57ee37a959bf507183c5dd6bf182081",
    token_id: "100",
    transform: {
      position: { x: 160 - 113.155, y: 3.439, z: 160 + 89.875 },
      rotation: { x: 0, y: 90 + 237.849, z: 0 },
      scale: { x: 8, y: 8, z: 4 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0xecf7ef42b57ee37a959bf507183c5dd6bf182081/100"
  },
  { // Repentance 19/20
    code: 8,
    contract_address: "0x22adcaf1d6b5213a76bfddab6d4d38c25ce7c029",
    token_id: "67",
    transform: {
      position: { x: 160 - 119.025, y: 3.44, z: 160 + 85.826 },
      rotation: { x: 0, y: 90 + 233.438, z: 0 },
      scale: { x: 8, y: 8, z: 4 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0x22adcaf1d6b5213a76bfddab6d4d38c25ce7c029/67"
  },
  { // 297 El Guapo
    code: 9,
    contract_address: "0x0163f249af5a4781cd09edeb2326066b9552d8ec",
    token_id: "9",
    transform: {
      position: { x: 160 - 125.044, y: 3.169, z: 160 + 81.033 },
      rotation: { x: 0, y: 90 + 226.024, z: 0 },
      scale: { x: 4, y: 4, z: 2 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0x0163f249af5a4781cd09edeb2326066b9552d8ec/9"
  },
  { // Robots are coming to Save the World
    code: 10,
    contract_address: "0xe977bfe8b83db27616ec10c7862066d37e0326a9",
    token_id: "7",
    transform: {
      position: { x: 160 - 130.01, y: 3.169, z: 160 + 75.49 },
      rotation: { x: 0, y: 90 + 222.012, z: 0 },
      scale: { x: 4, y: 4, z: 2 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0xe977bfe8b83db27616ec10c7862066d37e0326a9/7"
  },
  { // HARMONICS VIII
    code: 11,
    contract_address: "0xb32601bbc5136842b716a815d6cafe9a490f604d",
    token_id: "20",
    transform: {
      position: { x: 160 - 130.459, y: 3.169, z: 160 + 75.894 },
      rotation: { x: 0, y: 90 + 42.012, z: 0 },
      scale: { x: 4, y: 4, z: 2 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0xb32601bbc5136842b716a815d6cafe9a490f604d/20"
  },
  { // Cat Nap
    code: 12,
    contract_address: "0x1f0da6e1686d5c16b89f95ca4788431508a926f8",
    token_id: "1",
    transform: {
      position: { x: 160 - 125.42, y: 3.169, z: 160 + 81.422 },
      rotation: { x: 0, y: 90 + 46.024, z: 0 },
      scale: { x: 4, y: 4, z: 2 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0x1f0da6e1686d5c16b89f95ca4788431508a926f8/1"
  },
  { // Snake Tree
    code: 13,
    contract_address: "0x9f8c6346ca2e987342f301bd93b0c4b13819b234",
    token_id: "170",
    transform: {
      position: { x: 160 - 119.603, y: 3.44, z: 160 + 86.747 },
      rotation: { x: 0, y: 90 + 54.234, z: 0 },
      scale: { x: 8, y: 8, z: 4 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0x9f8c6346ca2e987342f301bd93b0c4b13819b234/170"
  },
  { // Monkey Tree
    code: 14,
    contract_address: "0x9f8c6346ca2e987342f301bd93b0c4b13819b234",
    token_id: "158",
    transform: {
      position: { x: 160 - 113.483, y: 3.44, z: 160 + 90.458 },
      rotation: { x: 0, y: 90 + 61.29, z: 0 },
      scale: { x: 8, y: 8, z: 4 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0x9f8c6346ca2e987342f301bd93b0c4b13819b234/158"
  },
  { // Trash Can Hobos
    code: 15,
    contract_address: "0x9f8c6346ca2e987342f301bd93b0c4b13819b234",
    token_id: "13",
    transform: {
      position: { x: 160 - 109.561, y: 3.169, z: 160 + 95.643 },
      rotation: { x: 0, y: 90 - 2.197 , z: 0 },
      scale: { x: 4, y: 4, z: 2 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0x9f8c6346ca2e987342f301bd93b0c4b13819b234/13"
  },
  { // Lady Tree
    code: 16,
    contract_address: "0x9f8c6346ca2e987342f301bd93b0c4b13819b234",
    token_id: "146",
    transform: {
      position: { x: 160 - 95.748, y: 3.169, z: 160 + 106.728 },
      rotation: { x: 0, y: 90 + 98.21, z: 0 },
      scale: { x: 4, y: 4, z: 2 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0x9f8c6346ca2e987342f301bd93b0c4b13819b234/146"
  },
  { // Skull by Skeenee 02 | Pinky
    code: 17,
    contract_address: "0xbc0e164ee423b7800e355b012c06446e28b1a29d",
    token_id: "1708",
    transform: {
      position: { x: 160 - 86.141, y: 3.169, z: 160 + 126.471 },
      rotation: { x: 0, y: 90 + 11.939, z: 0 },
      scale: { x: 4, y: 4, z: 2 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0xbc0e164ee423b7800e355b012c06446e28b1a29d/1708"
  },
  { // Rose Flip
    code: 18,
    contract_address: "0xbc0e164ee423b7800e355b012c06446e28b1a29d",
    token_id: "92",
    transform: {
      position: { x: 160 - 86.204, y: 3.566, z: 160 + 119.427 },
      rotation: { x: 0, y: 90 - 9.127, z: 0 },
      scale: { x: 6, y: 6, z: 3 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0xbc0e164ee423b7800e355b012c06446e28b1a29d/92"
  },
  { // Why So Blue?
    code: 19,
    contract_address: "0xbc0e164ee423b7800e355b012c06446e28b1a29d",
    token_id: "906",
    transform: {
      position: { x: 160 - 85.689, y: 3.169, z: 160 + 110.211 },
      rotation: { x: 0, y: 90 + 18.108, z: 0 },
      scale: { x: 4, y: 4, z: 2 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0xbc0e164ee423b7800e355b012c06446e28b1a29d/906"
  },
  { // NINJA​
    code: 20,
    contract_address: "0xbc0e164ee423b7800e355b012c06446e28b1a29d",
    token_id: "2549",
    transform: {
      position: { x: 160 - 87.839, y: 3.169, z: 160 + 104.362 },
      rotation: { x: 0, y: 90 + 25.679, z: 0 },
      scale: { x: 4, y: 4, z: 2 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0xbc0e164ee423b7800e355b012c06446e28b1a29d/2549"
  },
  { // It's Game Time
    code: 21,
    contract_address: "0x30ab12b93a31124333643cb6d55b96046b31aaa1",
    token_id: "82",
    transform: {
      position: { x: 160 - 99.655, y: 3.566, z: 160 + 91.934 },
      rotation: { x: 0, y: 90 + 72.184, z: 0 },
      scale: { x: 6, y: 6, z: 3 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0x30ab12b93a31124333643cb6d55b96046b31aaa1/82"
  },
  { // Sushi
    code: 22,
    contract_address: "0xcb1b2a805273115feb4e3bc13c3111d70afb2270",
    token_id: "107",
    transform: {
      position: { x: 160 - 107.884, y: 3.566, z: 160 + 93.945 },
      rotation: { x: 0, y: 90 + 149.519, z: 0 },
      scale: { x: 8, y: 8, z: 4 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0xcb1b2a805273115feb4e3bc13c3111d70afb2270/107"
  },
  { // Wander
    code: 23,
    contract_address: "0xcb1b2a805273115feb4e3bc13c3111d70afb2270",
    token_id: "9",
    transform: {
      position: { x: 160 - 104.704, y: 3.566, z: 160 + 102.608 },
      rotation: { x: 0, y: 90 + 232.96, z: 0 },
      scale: { x: 6, y: 6, z: 3 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0xcb1b2a805273115feb4e3bc13c3111d70afb2270/9"
  },
  { // Day Fall
    code: 24,
    contract_address: "0xcb1b2a805273115feb4e3bc13c3111d70afb2270",
    token_id: "117",
    transform: {
      position: { x: 160 - 87.104, y: 9.716, z: 160 + 133.303 },
      rotation: { x: 0, y: 90 + 180, z: 0 },
      scale: { x: 6, y: 6, z: 3 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0xcb1b2a805273115feb4e3bc13c3111d70afb2270/117"
  },
  { // PXLPET 03/10 [ABSTRACT HOME EDITION]
    code: 25,
    contract_address: "0x2a46f2ffd99e19a89476e2f62270e0a35bbf0756",
    token_id: "23838",
    transform: {
      position: { x: 160 - 87.089, y: 9.716, z: 160 + 126.902 },
      rotation: { x: 0, y: 90 + 180, z: 0 },
      scale: { x: 6, y: 6, z: 3 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0x2a46f2ffd99e19a89476e2f62270e0a35bbf0756/23838"
  },
  { // Alice in Water
    code: 26,
    contract_address: "0x2a46f2ffd99e19a89476e2f62270e0a35bbf0756",
    token_id: "23972",
    transform: {
      position: { x: 160 - 87.075, y: 9.716, z: 160 + 120.827 },
      rotation: { x: 0, y: 90 + 180, z: 0 },
      scale: { x: 6, y: 6, z: 3 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0x2a46f2ffd99e19a89476e2f62270e0a35bbf0756/23972"
  },
  { // Teddy Machine
    code: 27,
    contract_address: "0x2a46f2ffd99e19a89476e2f62270e0a35bbf0756",
    token_id: "18274",
    transform: {
      position: { x: 160 - 87.061, y: 9.716, z: 160 + 115.246 },
      rotation: { x: 0, y: 90 + 180, z: 0 },
      scale: { x: 6, y: 6, z: 3 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0x2a46f2ffd99e19a89476e2f62270e0a35bbf0756/18274"
  },
  { // The Tempest and the Cliff
    code: 28,
    contract_address: "0x2a46f2ffd99e19a89476e2f62270e0a35bbf0756",
    token_id: "21739",
    transform: {
      position: { x: 160 - 89.537, y: 9.365, z: 160 + 104.19 },
      rotation: { x: 0, y: 90 - 77.139, z: 0 },
      scale: { x: 4, y: 4, z: 2 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0x2a46f2ffd99e19a89476e2f62270e0a35bbf0756/21739"
  },
  { // If it does not touch my skin...
    code: 29,
    contract_address: "0xfbeef911dc5821886e1dda71586d90ed28174b7d",
    token_id: "66152",
    transform: {
      position: { x: 160 - 98.988, y: 9.365, z: 160 + 101.345 },
      rotation: { x: 0, y: 90 + 230.096, z: 0 },
      scale: { x: 4, y: 4, z: 2 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0xfbeef911dc5821886e1dda71586d90ed28174b7d/66152"
  },
  { // Pressure those thoughts.
    code: 30,
    contract_address: "0xfbeef911dc5821886e1dda71586d90ed28174b7d",
    token_id: "163326",
    transform: {
      position: { x: 160 - 102.329, y: 9.365, z: 160 + 98.461 },
      rotation: { x: 0, y: 90 + 228.444, z: 0 },
      scale: { x: 4, y: 4, z: 2 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0xfbeef911dc5821886e1dda71586d90ed28174b7d/163326"
  },
  { // Lost Redemption
    code: 31,
    contract_address: "0xfbeef911dc5821886e1dda71586d90ed28174b7d",
    token_id: "83126",
    transform: {
      position: { x: 160 - 105.744, y: 9.365, z: 160 + 95.934 },
      rotation: { x: 0, y: 90 + 233.524, z: 0 },
      scale: { x: 4, y: 4, z: 2 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0xfbeef911dc5821886e1dda71586d90ed28174b7d"
  },
  { // The Deadness
    code: 32,
    contract_address: "0xfbeef911dc5821886e1dda71586d90ed28174b7d",
    token_id: "105501",
    transform: {
      position: { x: 160 - 106.163, y: 9.365, z: 160 + 96.298 },
      rotation: { x: 0, y: 90 + 54.323, z: 0 },
      scale: { x: 4, y: 4, z: 2 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0xfbeef911dc5821886e1dda71586d90ed28174b7d/105501"
  },
  { // Choose the Man You Will Become
    code: 33,
    contract_address: "0x6c424c25e9f1fff9642cb5b7750b0db7312c29ad",
    token_id: "118",
    transform: {
      position: { x: 160 - 102.632, y: 9.365, z: 160 + 98.918 },
      rotation: { x: 0, y: 90 + 48.444, z: 0 },
      scale: { x: 4, y: 4, z: 2 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0x6c424c25e9f1fff9642cb5b7750b0db7312c29ad/118"
  },
  { // Virtual Vitruvian
    code: 34,
    contract_address: "0x6c424c25e9f1fff9642cb5b7750b0db7312c29ad",
    token_id: "100",
    transform: {
      position: { x: 160 - 99.441, y: 9.365, z: 160 + 101.747 },
      rotation: { x: 0, y: 90 + 48.444, z: 0 },
      scale: { x: 4, y: 4, z: 2 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0x6c424c25e9f1fff9642cb5b7750b0db7312c29ad/100"
  },
  { // PRESENT
    code: 35,
    contract_address: "0x4679b43e6ebcb2e7fca0f4e943e6231b87cf29f7",
    token_id: "11",
    transform: {
      position: { x: 160 - 92.731, y: 9.365, z: 160 + 105.886 },
      rotation: { x: 0, y: 90 + 116.174, z: 0 },
      scale: { x: 4, y: 4, z: 2 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0x4679b43e6ebcb2e7fca0f4e943e6231b87cf29f7/11"
  },
  { // Forms of Love
    code: 36,
    contract_address: "0x6c424c25e9f1fff9642cb5b7750b0db7312c29ad",
    token_id: "59",
    transform: {
      position: { x: 160 - 88.017, y: 9.365, z: 160 + 116.22 },
      rotation: { x: 0, y: 90, z: 0 },
      scale: { x: 4, y: 4, z: 2 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0x6c424c25e9f1fff9642cb5b7750b0db7312c29ad/59"
  },
  { // DCL DemoN
    code: 37,
    contract_address: "0xbc0e164ee423b7800e355b012c06446e28b1a29d",
    token_id: "2541",
    transform: {
      position: { x: 160 - 88.017, y: 9.365, z: 160 + 121.79 },
      rotation: { x: 0, y: 90, z: 0 },
      scale: { x: 4, y: 4, z: 2 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0xbc0e164ee423b7800e355b012c06446e28b1a29d/2541"
  },
  { // Podmork PIX18
    code: 38,
    contract_address: "0xbc0e164ee423b7800e355b012c06446e28b1a29d",
    token_id: "2542",
    transform: {
      position: { x: 160 - 88.017, y: 9.365, z: 160 + 127.626 },
      rotation: { x: 0, y: 90, z: 0 },
      scale: { x: 4, y: 4, z: 2 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0xbc0e164ee423b7800e355b012c06446e28b1a29d/2542"
  },
  { // Crypto Combat
    code: 39,
    contract_address: "0x2a46f2ffd99e19a89476e2f62270e0a35bbf0756",
    token_id: "24982",
    transform: {
      position: { x: 160 - 88.017, y: 9.365, z: 160 + 133.626 },
      rotation: { x: 0, y: 90, z: 0 },
      scale: { x: 4, y: 4, z: 2 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0x2a46f2ffd99e19a89476e2f62270e0a35bbf0756/24982"
  },

  /// --- Museum B ---
  { // POAP Genesis Plaza
    code: 1,
    contract_address: "0x22c1f6050e56d2876009903609a2cc3fef83b415",
    token_id: "11930",
    transform: {
      position: { x: 160 - 118.873, y: 8.9, z: 160 - 8.845 },
      rotation: { x: 0, y: 90 + 60, z: 0 },
      scale: { x: 6, y: 6, z: 3 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0x8dc3be37a1d83a174a4d417980bcb455e347ba65/11930"
  },
  { // POAP SuperRare
    code: 2,
    contract_address: "0x22c1f6050e56d2876009903609a2cc3fef83b415",
    token_id: "12129",
    transform: {
      position: { x: 160 - 134.691, y: 8.9, z: 160 - 9.033 },
      rotation: { x: 0, y: 90 + 120, z: 0 },
      scale: { x: 6, y: 6, z: 3 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0x22c1f6050e56d2876009903609a2cc3fef83b415/12129"
  },
  { // POAP OpenSea
    code: 3,
    contract_address: "0x22c1f6050e56d2876009903609a2cc3fef83b415",
    token_id: "12385",
    transform: {
      position: { x: 160 - 142.763, y: 8.9, z: 160 + 4.571 },
      rotation: { x: 0, y: 90 + 180, z: 0 },
      scale: { x: 6, y: 6, z: 3 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0x22c1f6050e56d2876009903609a2cc3fef83b415/12129"
  },
  { // POAP Mintbase
    code: 4,
    contract_address: "0x22c1f6050e56d2876009903609a2cc3fef83b415",
    token_id: "12564",
    transform: {
      position: { x: 160 - 135.017, y: 8.9, z: 160 + 18.364 },
      rotation: { x: 0, y: 90 + 240, z: 0 },
      scale: { x: 6, y: 6, z: 3 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0x22c1f6050e56d2876009903609a2cc3fef83b415/12564"
  },
  { // POAP Abner Harris
    code: 5,
    contract_address: "0x22c1f6050e56d2876009903609a2cc3fef83b415",
    token_id: "12751",
    transform: {
      position: { x: 160 - 119.199, y: 8.9, z: 160 + 18.552 },
      rotation: { x: 0, y: 90 - 60, z: 0 },
      scale: { x: 6, y: 6, z: 3 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0x22c1f6050e56d2876009903609a2cc3fef83b415/12751"
  },
  { // POAP PixelChain
    code: 6,
    contract_address: "0x22c1f6050e56d2876009903609a2cc3fef83b415",
    token_id: "12903",
    transform: {
      position: { x: 160 - 111.127, y: 8.9, z: 160 + 4.947 },
      rotation: { x: 0, y: 90, z: 0 },
      scale: { x: 6, y: 6, z: 3 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0x22c1f6050e56d2876009903609a2cc3fef83b415/12903"
  },
  { // POAP ChainGuardians
    code: 7,
    contract_address: "0x22c1f6050e56d2876009903609a2cc3fef83b415",
    token_id: "13054",
    transform: {
      position: { x: 160 - 117.83, y: 15.271, z: 160 - 10.604 },
      rotation: { x: 0, y: 90 + 60, z: 0 },
      scale: { x: 5, y: 5, z: 2.5 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0x22c1f6050e56d2876009903609a2cc3fef83b415/13054"
  },
  { // POAP MakersPlace
    code: 8,
    contract_address: "0x22c1f6050e56d2876009903609a2cc3fef83b415",
    token_id: "13232",
    transform: {
      position: { x: 160 - 126.733, y: 15.271, z: 160 - 13.104 },
      rotation: { x: 0, y: 90 + 90, z: 0 },
      scale: { x: 5, y: 5, z: 2.5 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0x22c1f6050e56d2876009903609a2cc3fef83b415/13232"
  },
  { // POAP Known Origin (REPLACE)
    code: 9,
    contract_address: "0x22c1f6050e56d2876009903609a2cc3fef83b415",
    token_id: "13421",
    transform: {
      position: { x: 160 - 135.693, y: 15.271, z: 160 - 10.817 },
      rotation: { x: 0, y: 90 + 120, z: 0 },
      scale: { x: 5, y: 5, z: 2.5 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0x22c1f6050e56d2876009903609a2cc3fef83b415/13421"
  },
  { // POAP Soho
    code: 10,
    contract_address: "0x22c1f6050e56d2876009903609a2cc3fef83b415",
    token_id: "13661",
    transform: {
      position: { x: 160 - 142.309, y: 15.271, z: 160 - 4.356 },
      rotation: { x: 0, y: 90 + 150, z: 0 },
      scale: { x: 5, y: 5, z: 2.5 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0x22c1f6050e56d2876009903609a2cc3fef83b415/13661"
  },
  { // POAP Async
    code: 11,
    contract_address: "0x22c1f6050e56d2876009903609a2cc3fef83b415",
    token_id: "13633",
    transform: {
      position: { x: 160 - 144.808, y: 15.271, z: 160 + 4.547 },
      rotation: { x: 0, y: 90 + 180, z: 0 },
      scale: { x: 5, y: 5, z: 2.5 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0x22c1f6050e56d2876009903609a2cc3fef83b415/13633"
  },
  { // CryptoMorph Decentraland
    code: 12,
    contract_address: "0x8dc3be37a1d83a174a4d417980bcb455e347ba65",
    token_id: "40",
    transform: {
      position: { x: 160 - 142.521, y: 15.271, z: 160 + 13.507 },
      rotation: { x: 0, y: 90 + 210, z: 0 },
      scale: { x: 5, y: 5, z: 2.5 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0x8dc3be37a1d83a174a4d417980bcb455e347ba65/40"
  },
  { // PixelChain Decentraland
    code: 13,
    contract_address: "0xbc0e164ee423b7800e355b012c06446e28b1a29d",
    token_id: "1021",
    transform: {
      position: { x: 160 - 136.06, y: 15.271, z: 160 + 20.123 },
      rotation: { x: 0, y: 90 + 240, z: 0 },
      scale: { x: 5, y: 5, z: 2.5 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0xbc0e164ee423b7800e355b012c06446e28b1a29d/1021"
  },
  { // Girl in Spy Suit and Ice Cream Hat
    code: 14,
    contract_address: "0x4da54e1fa95b7f28fb9ff1df06c89070c1ab0b03",
    token_id: "7",
    transform: {
      position: { x: 160 - 127.157, y: 15.271, z: 160 + 22.623 },
      rotation: { x: 0, y: 0, z: 0 },
      scale: { x: 5, y: 5, z: 2.5 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0x4da54e1fa95b7f28fb9ff1df06c89070c1ab0b03/7"
  },
  { // DCL - Open the Gates
    code: 15,
    contract_address: "0xfbeef911dc5821886e1dda71586d90ed28174b7d",
    token_id: "136972",
    transform: {
      position: { x: 160 - 118.197, y: 15.271, z: 160 + 20.336 },
      rotation: { x: 0, y: 90 - 60, z: 0 },
      scale: { x: 5, y: 5, z: 2.5 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0xfbeef911dc5821886e1dda71586d90ed28174b7d/136972"
  },
  { // MANA-FEVER.FUN​
    code: 16,
    contract_address: "0xbc0e164ee423b7800e355b012c06446e28b1a29d",
    token_id: "2546",
    transform: {
      position: { x: 160 - 111.581, y: 15.271, z: 160 + 13.875 },
      rotation: { x: 0, y: 90 - 30, z: 0 },
      scale: { x: 5, y: 5, z: 2.5 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0xbc0e164ee423b7800e355b012c06446e28b1a29d/2546"
  },
  { // Tune In #12
    code: 17,
    contract_address: "0xecf7ef42b57ee37a959bf507183c5dd6bf182081",
    token_id: "13",
    transform: {
      position: { x: 160 - 109.082, y: 15.271, z: 160 + 4.972 },
      rotation: { x: 0, y: 0 + 90, z: 0 },
      scale: { x: 5, y: 5, z: 2.5 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0xecf7ef42b57ee37a959bf507183c5dd6bf182081/13"
  },
  { // Shibu's Masterpiece (REPLACE)
    code: 18,
    contract_address: "0x4679b43e6ebcb2e7fca0f4e943e6231b87cf29f7",
    token_id: "1",
    transform: {
      position: { x: 160 - 111.369, y: 15.271, z: 160 - 3.988 },
      rotation: { x: 0, y: 90 + 30, z: 0 },
      scale: { x: 5, y: 5, z: 2.5 }
    },
    style: 18,
    color: Color3.White(),
    link: "https://opensea.io/assets/0x4679b43e6ebcb2e7fca0f4e943e6231b87cf29f7/1"
  },

  /// --- Billboards ---
  { // Unstable Condition
    code: 27,
    contract_address: "0x2a46f2ffd99e19a89476e2f62270e0a35bbf0756",
    token_id: "22754",
    transform: {
      position: { x: 160 + 14.935, y: 17.918, z: 160 - 0.265 },
      rotation: { x: 0, y: 90 - 37.521, z: 0 },
      scale: { x: 14, y: 14, z: 14 }
    },
    style: 21,
    color: Color3.Black(),
    link: "https://opensea.io/assets/0x2a46f2ffd99e19a89476e2f62270e0a35bbf0756/22754"
  },
  { // Machine Sea Horse
    code: 27,
    contract_address: "0x2a46f2ffd99e19a89476e2f62270e0a35bbf0756",
    token_id: "24961",
    transform: {
      position: { x: 160 + 17.671, y: 17.918, z: 160 + 1.836 },
      rotation: { x: 0, y: 232.479, z: 0 },
      scale: { x: 14, y: 14, z: 14 }
    },
    style: 21,
    color: Color3.Black(),
    link: "https://opensea.io/assets/0x2a46f2ffd99e19a89476e2f62270e0a35bbf0756/24961"
  },
  { // Take Control or Taken
    code: 27,
    contract_address: "0x2a46f2ffd99e19a89476e2f62270e0a35bbf0756",
    token_id: "20644",
    transform: {
      position: { x: 160 - 55.495, y: 16.12, z: 160 - 55.853 },
      rotation: { x: 0, y: 90 + 142.479, z: 0 },
      scale: { x: 11, y: 11, z: 11 }
    },
    style: 21,
    color: Color3.Black(),
    link: "https://opensea.io/assets/0x2a46f2ffd99e19a89476e2f62270e0a35bbf0756/20644"
  },
  { // The Hunter
    code: 27,
    contract_address: "0x2a46f2ffd99e19a89476e2f62270e0a35bbf0756",
    token_id: "24960",
    transform: {
      position: { x: 160 - 57.648, y: 16.12, z: 160 - 57.507 },
      rotation: { x: 0, y: 52.479, z: 0 },
      scale: { x: 11, y: 11, z: 11 }
    },
    style: 21,
    color: Color3.Black(),
    link: "https://opensea.io/assets/0x2a46f2ffd99e19a89476e2f62270e0a35bbf0756/24960"
  },
]
